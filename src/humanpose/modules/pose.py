...
class Pose:
    ...
    def draw_hand(self, img):
        ...
        if global_kpt_b_id != -1:
            lwrist_X, lwrist_Y = self.keypoints[7]
            lelbow_X, lelbow_Y = self.keypoints[6]

            distance_forearm = math.sqrt(((lwrist_X-lelbow_X)**2)+((lwrist_Y-lelbow_Y)**2))

            print(distance_forearm)
            cv2.circle(img, (int(lwrist_X), int(lwrist_Y)), 23, (0, 0, 255), -1)
